...
For matched pairs, it mirrors the YNAB UI behavior: if one side has an `import_payee_name` value that is a JSON object,
that row is deleted and the other row is approved. Otherwise, matching rows are approved. Scheduled transaction matches
are approved.

Preview mode prints each transaction with an action of `Update` or `Delete`. Applying changes runs the YNAB API requests
concurrently, limited to five requests at a time.
