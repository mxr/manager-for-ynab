...
from manager_for_ynab.add_transaction import run as run_add_txn
...
_ADD_TXN_HELP = "Create a transaction and optionally fund a category."
...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_txn_parser = subparsers.add_parser(
        "add-transaction",
        help=_ADD_TXN_HELP,
        description=_ADD_TXN_HELP,
    )
    add_txn_parser.set_defaults(func=run_add_txn)
...
async def async_main(argv: Sequence[str] = ()) -> int:
    ...
    match argv[0]:
        ...
        case "add-transaction":
            return await run_add_txn(argv[1:])
        case "zero-out":
