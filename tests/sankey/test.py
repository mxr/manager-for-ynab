...
def test_build_sankey_data_uses_sql_netted_category_outflows():
    ...
    assert data.labels == ["Ready to Assign", "Gifts", "Gifts"]
...
def test_build_sankey_data_treats_sql_netted_category_income_by_category():
    ...
    assert data.labels == ["Gifts", "Ready to Assign", "Net Category Income"]
    assert data.sources == [0, 2]
    assert data.targets == [2, 1]
...
def test_build_sankey_data_keeps_payee_income_separate_from_net_category_income():
    ...
    assert data.sources == [0, 1, 3]
    assert data.targets == [2, 3, 2]
    assert data.values == [Decimal("500"), Decimal("60"), Decimal("60")]
